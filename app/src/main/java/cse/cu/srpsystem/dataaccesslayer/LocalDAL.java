...
public class LocalDAL {
    ...
    private static LocalDAL handler;

    private LocalDAL(Context context) {
        DBHelper helper = new DBHelper(context);
        wdb = helper.getWritableDatabase();
        rdb = helper.getReadableDatabase();
    }

    public static LocalDAL getInstance(Context context) {
        if (handler == null)
            handler = new LocalDAL(context);
        return handler;
    }
    ...
    public void updateLocalData(EntityTypeName entityTypeName, List<?> list) {
    ...
    public List<Result> getMarksAsResult(int exam_id) {
        ...
        for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
            ...
            result.gpa = CalculateGPABL.getGPA(cursor.getFloat(4), cursor.getFloat(3));
